                ...
                if(err) {
                    ...
                    if( res.statusCode>=100 && res.statusCode<400 ) {
                        ...
                        if( result.errors && result.errors.length>0 ) {
                            throw new Error("The response for "+opt.url+" returned errors " + JSON.stringify(result.errors));
                        }
                        if( !result.data ) {
                            throw new Error("Unexpected result with no data. "+ JSON.stringify(result));
                        }
                        resolve(result.data);
                    } else {
